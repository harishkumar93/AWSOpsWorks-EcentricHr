...
function job_description()
{
  ...
  exit;*/
  //$id = explode("?",$url[3]);
//	$result = $wpdb->get_results("SELECT * FROM wp_jobs where permalink='$url[1]'");
...
function current_openings_outsource()
{
      ...
      foreach ($result as $key => $value) 
      {
         ...
         // $url = 'http://ecentrichr.com/careers/job/?'."$id";
          ...
          $url =  $ecentricUrl.'/careers/job/?'."$id";
...
{
  global $wpdb;
  $result = $wpdb->get_results("SELECT * FROM wp_jobs where status=1 AND job_type=2 order by id desc limit 0,11");
  $cnt = 1;
  if(count($result)!=0)
  { 
    foreach ($result as $key => $value) 
      {
        ...
        $ecentricUrl = get_site_url();
      	$url =  $ecentricUrl.'/careers/job/?'."$id";
        $row .="<a href='$url'><li>$value->job_title</li></a>";
        $cnt++;
      }
  }
  else
  ...
  $message = <<<Message
      ...
      <a href="http://ecentrichr.com/careers/current-openings/"><div class="header-text">Current Openings</div></a>
        ...
        <a href="http://ecentrichr.com/careers/current-openings/"><h6>More</h6></a>
<br style="clear:both;">
    </div>
Message;
return $message;
